...
// Generation of types, properties, map, cell and clues.
...
//It generates a specific cell. w gives the index of types and x is the set of index of properties for the cel.l
...
// It build a public game master key and secret game master key.   	
pub fn buildmaster< T: CryptoRng + RngCore>(csprng: &mut T, genmaster: RistrettoPoint, tj: RistrettoPoint, pj: Vec<RistrettoPoint>, p: Vec<RistrettoPoint>) -> (Vec<RistrettoPoint>, Scalar){
    ...
    let key = gen_elgamal(csprng, genmaster); // key.0 = genmaster, key.1 = pk, key.2 = sk
...
// It build a set of random cell depending on the types and properties generated. n is the number of cell.
...
pub fn measuresvcc<T: CryptoRng + RngCore>(csprng: &mut T, genmaster: RistrettoPoint, genplayers: Vec<RistrettoPoint>, cluesforms: Vec<usize>, n: usize, iter: u32){
    
    let mut sumprovegame = Duration::ZERO;
    let mut sumverifygame = Duration::ZERO;
    let mut proofgamesize = 0;
    
    println!("Average of the running time of the algorithms ProveGame and VerifyGame, and average of the size of the proofs over {:?} iterations.\n", iter);   
    ...
    for _j in 0..iter {
        ...
        let t = maps.0; // Vector of 5 types.
        let p = maps.1; // Vector of 14 properties.
        let bottom = maps.2;
...
pub fn prove_game2<T: CryptoRng + RngCore>(csprng: &mut T, y0: RistrettoPoint, ya: RistrettoPoint, y: Vec<RistrettoPoint>, g0: RistrettoPoint, ga:RistrettoPoint, g: Vec<RistrettoPoint>, h: Vec<RistrettoPoint>, x: Scalar, xa: Scalar) -> Proofgame2{
    ...
    let mut k = 0;
    while k < y.len() && (y[k] != x * g[k] + xa * h[k]){ // y = x * g + xa * h.
        k = k + 1;
    }
    let mut w = k; 
    if w == y.len(){
...
pub fn prove_game3<T: CryptoRng + RngCore>(csprng: &mut T, y0: RistrettoPoint, y1: RistrettoPoint, y2: RistrettoPoint, y3: Vec<RistrettoPoint>, y4: Vec<RistrettoPoint>, y5: RistrettoPoint, y6: RistrettoPoint, y7: Vec<RistrettoPoint>, g0: RistrettoPoint, g1: RistrettoPoint, g2: RistrettoPoint, g3: Vec<RistrettoPoint>, g4: Vec<RistrettoPoint>, g5: RistrettoPoint, g6: RistrettoPoint, g7: Vec<RistrettoPoint>, x: Scalar) -> Proofgame3{
    ...
    // To determine which part of the proof is true.
    let mut truepart1 = 0;
    let mut truepart3 = 0;
    let mut truepart4 = 0;
    ...
    else{ // The second part of proof is true or it build a false proof.
...
pub fn prove_or<T: CryptoRng + RngCore>(csprng: &mut T, y: Vec<RistrettoPoint>, g: Vec<RistrettoPoint>, x: Scalar) -> (Vec<RistrettoPoint>, Vec<Scalar>, Vec<Scalar>, usize) {
    ...
    let mut k = 0;
    while k < y.len() && (y[k] != x * g[k]){ // y = x * g + xa * h.
        k = k + 1;
    }
    let mut w = k; 
    if w == y.len(){
